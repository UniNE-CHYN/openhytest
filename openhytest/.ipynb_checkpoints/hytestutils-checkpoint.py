...
# _____________________
#
# This file contains basic utilities to pre and post-process
# well test data, including resampling, filtering and data visualization.
...
class HyTestUtils():
    """Parent class providingall the basic utilities
    ...
    def header(self):
        """Reads the header of the pandas df dataframe."""

        hd = self.df.head(1)
        self.hd = list(hd)

    def ldiff(self, df=None):
        """Computes the logarithmic derivative with centered difference scheme.

        Parameters
        ----------
        df : pandas df expects two columns in the dataframe.
              The first column is elapsed time (df.t)
              The second df trace needs to be sampled at the given
              time in the first column (df.s)

        Returns
        -------
        der : array
            logarithmic derivative in pandas dataframe format with the same names given by the input df.
    ...
    def ldiff_plot(self, df=None):
        """Plots the logarithmic derivative with centered difference scheme.

        Parameters
        ----------
        df : pandas df
            expects two vectors with t and s
        """
    ...
    def ldiffs(self, df=None, npoints=None):
        """Computes the logarithmic derivative of x with 1D linear interpolation of y
